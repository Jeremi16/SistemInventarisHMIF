...
<body class="min-h-screen bg-[#f7faf7] font-sans text-gray-900 antialiased">
            ...
            <div class="w-full max-w-md">
                ...
                <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm sm:p-8">
                    <div class="mb-7">
                        ...
                        <p class="mt-2 text-sm leading-6 text-gray-500">Gunakan email/NIM terdaftar, Google student yang mengandung NIM pola 1xx140, atau email if.itera.ac.id.</p>
                    </div>
                    ...
                    @if(app()->isLocal() && session('google_login_debug'))
                        <div class="mb-5 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                            <p class="font-semibold">Debug OAuth Google</p>
                            <p class="mt-1 text-xs leading-5 text-amber-800">Detail ini hanya tampil di local dan juga dikirim ke browser console.</p>
                            <pre class="mt-3 max-h-56 overflow-auto whitespace-pre-wrap break-words rounded-md bg-white/70 p-3 text-xs text-amber-950">{{ json_encode(session('google_login_debug'), JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES) }}</pre>
                        </div>
                    @endif

                    <a
                        href="{{ route('auth.google.redirect') }}"
                        class="inline-flex w-full items-center justify-center gap-3 rounded-lg border border-gray-200 bg-white px-5 py-3 text-sm font-semibold text-gray-700 shadow-sm transition hover:border-[#1b8a1d] hover:bg-green-50 hover:text-[#176d19] focus:outline-none focus:ring-4 focus:ring-green-100"
                    >
                        <svg class="h-5 w-5 flex-none" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <div class="my-6 flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.14em] text-gray-400">
    ...
    @if(app()->isLocal() && session('google_login_debug'))
        <script>
            console.warn('Google OAuth debug', @json(session('google_login_debug')));
        </script>
    @endif
</body>
</html>
