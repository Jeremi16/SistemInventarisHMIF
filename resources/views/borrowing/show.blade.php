...
@php
    $statusConfig = [
        ...
        'approved' => ['label' => 'Siap Diambil', 'class' => 'bg-hmif-100 text-hmif-800 border-hmif-200'],
        'rejected' => ['label' => 'Ditolak', 'class' => 'bg-red-100 text-red-800 border-red-200'],
        'borrowed' => ['label' => 'Dipinjam', 'class' => 'bg-green-100 text-green-800 border-green-200'],
...
<div class="mx-auto max-w-5xl space-y-6">
    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div class="min-w-0">
            ...
            <h2 class="hmif-break-anywhere mt-2 text-xl font-bold text-gray-900 sm:text-2xl">{{ $borrowing->item_name }}</h2>
            <p class="mt-1 text-sm text-gray-500">Pengajuan #{{ $borrowing->id }} oleh {{ $borrowing->borrower_name }}</p>
        </div>
        <div class="hmif-mobile-actions sm:justify-end">
    ...
    <section class="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2">
                <div class="rounded-lg bg-gray-50 p-4">
                    ...
                    <p class="hmif-break-anywhere mt-2 font-semibold text-gray-900">{{ $borrowing->startDateTime()?->format('d M Y H:i:s') }}</p>
                </div>
                <div class="rounded-lg bg-gray-50 p-4">
                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-500">Tanggal Kembali</p>
                    <p class="hmif-break-anywhere mt-2 font-semibold text-gray-900">{{ $borrowing->endDateTime()?->format('d M Y H:i:s') }}</p>
                </div>
            </div>

            <div class="mt-6">
                <p class="text-sm font-semibold text-gray-900">Keperluan</p>
                <p class="mt-2 min-h-28 whitespace-pre-line break-words rounded-lg bg-gray-50 p-4 text-sm leading-6 text-gray-700">{{ $borrowing->purpose }}</p>
            </div>

            <div class="mt-6">
                <p class="text-sm font-semibold text-gray-900">Catatan Admin</p>
                <p class="mt-2 min-h-24 whitespace-pre-line break-words rounded-lg bg-hmif-50 p-4 text-sm leading-6 text-hmif-900">
            ...
            <div class="mt-6 grid gap-4 sm:grid-cols-2">
                <div class="rounded-lg bg-gray-50 p-4">
                    ...
                    @endif
                    @if($borrowing->handover_photo)
                        <a href="{{ asset('storage/' . $borrowing->handover_photo) }}" target="_blank" rel="noopener noreferrer" class="mt-3 block overflow-hidden rounded-lg border border-gray-200">
                            <img src="{{ asset('storage/' . $borrowing->handover_photo) }}" alt="Foto penyerahan {{ $borrowing->item_name }}" class="h-32 w-full object-cover">
                        </a>
                    @endif
                </div>
                <div class="rounded-lg bg-gray-50 p-4">
                    ...
                    @if($borrowing->return_photo)
                        <a href="{{ asset('storage/' . $borrowing->return_photo) }}" target="_blank" rel="noopener noreferrer" class="mt-3 block overflow-hidden rounded-lg border border-gray-200">
                            <img src="{{ asset('storage/' . $borrowing->return_photo) }}" alt="Foto pengembalian {{ $borrowing->item_name }}" class="h-32 w-full object-cover">
                        </a>
                    @endif
                </div>
            </div>
            ...
            @if($borrowing->damage_description)
                <div class="mt-6">
                    ...
                    <p class="mt-2 whitespace-pre-line break-words rounded-lg bg-red-50 p-4 text-sm leading-6 text-red-800">{{ $borrowing->damage_description }}</p>
                </div>
            @endif
        </div>
        ...
        <aside class="space-y-6">
            <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
                ...
                <div class="mt-4 space-y-4 text-sm">
                    <div>
                        ...
                        <p class="hmif-break-anywhere mt-1 font-semibold text-gray-900">{{ $borrowing->borrower_name }}</p>
                    </div>
                    <div>
                        <p class="text-gray-500">NIM</p>
                        <p class="hmif-break-anywhere mt-1 font-semibold text-gray-900">{{ $borrowing->borrower_nim ?? 'NIM belum tersedia' }}</p>
                    </div>
                </div>
            </div>

            <div class="theme-note-panel rounded-xl border border-[#d7e78a] bg-[#fbfde8] p-4 shadow-sm sm:p-6">
            ...
            @if($borrowing->extension_requested)
                <div class="rounded-xl border border-hmif-200 bg-hmif-50 p-4 shadow-sm sm:p-6">
                    <h3 class="font-semibold text-hmif-900">Permintaan Perpanjangan</h3>
                    <p class="mt-2 text-sm font-medium text-hmif-900">Tanggal baru: {{ $borrowing->extension_new_date?->format('d M Y') }}</p>
                    <p class="mt-2 whitespace-pre-line break-words text-sm leading-6 text-hmif-900">{{ $borrowing->extension_reason }}</p>
                    @if($isAdmin)
                        <div class="mt-4 flex flex-col gap-2">
                            <form method="POST" action="{{ route('borrowing.extension.approve', $borrowing) }}">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="w-full rounded-lg bg-hmif-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-hmif-700">Setujui</button>
                            </form>
                            <form method="POST" action="{{ route('borrowing.extension.reject', $borrowing) }}" class="space-y-2">
                                @csrf
                                @method('PATCH')
                                <textarea name="extension_rejection_reason" rows="3" required class="w-full rounded-lg border border-hmif-200 bg-white px-3 py-2 text-sm text-gray-800 focus:border-hmif-500 focus:outline-none focus:ring-2 focus:ring-hmif-100" placeholder="Alasan penolakan perpanjangan">{{ old('extension_rejection_reason') }}</textarea>
            ...
            @if($borrowing->extension_rejection_reason)
                <div class="rounded-xl border border-red-200 bg-red-50 p-4 shadow-sm sm:p-6">
                    <h3 class="font-semibold text-red-900">Perpanjangan Ditolak</h3>
                    <p class="mt-2 text-xs font-medium uppercase tracking-wide text-red-700">
                        {{ $borrowing->extension_rejected_at?->format('d M Y H:i:s') ?? 'Baru diperbarui' }}
                    </p>
                    <p class="mt-3 whitespace-pre-line break-words text-sm leading-6 text-red-800">{{ $borrowing->extension_rejection_reason }}</p>
                </div>
            @endif
            ...
            @if($isAdmin && in_array($borrowing->status, ['borrowed', 'overdue'], true))
                <div class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
                    ...
                    <form method="POST" action="{{ route('borrowing.pre-return', $borrowing) }}" class="mt-4 space-y-3">
                        ...
                        <input type="date" name="pre_return_check_date" value="{{ old('pre_return_check_date', $borrowing->pre_return_check_date?->format('Y-m-d') ?? now()->toDateString()) }}" min="{{ ($borrowing->handover_date ?: $borrowing->start_date)->format('Y-m-d') }}" max="{{ now()->toDateString() }}" required class="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm focus:border-hmif-500 focus:outline-none focus:ring-2 focus:ring-hmif-100">
    ...
    <section class="rounded-xl border border-gray-100 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div class="mt-6 divide-y divide-gray-100">
            @forelse($borrowing->notes as $note)
                <article class="flex flex-col gap-3 py-4 sm:flex-row sm:gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <p class="mt-2 whitespace-pre-line break-words text-sm leading-6 text-gray-700">{{ $note->content }}</p>
                    </div>
                    @if($isAdmin)
                        <form method="POST" action="{{ route('borrowing.notes.delete', $note) }}" onsubmit="return confirm('Hapus catatan ini?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full rounded-lg border border-red-200 px-3 py-2 text-sm font-semibold text-red-700 transition hover:bg-red-50 sm:w-auto">Hapus</button>
                        </form>
                    @endif
                </article>
            @empty
