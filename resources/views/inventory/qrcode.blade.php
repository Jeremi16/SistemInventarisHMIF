...
<div class="mx-auto max-w-3xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Pindai untuk membuka detail {{ $item->name }}.</p>
        </div>
    ...
    <div class="rounded-xl border border-gray-100 bg-white p-6 text-center shadow-sm">
        <img src="{{ $qrUrl }}" alt="QR Code {{ $item->name }}" class="mx-auto h-72 w-72 rounded-lg border border-gray-100 bg-white p-3">
        <h3 class="mt-5 text-lg font-semibold text-gray-900">{{ $item->name }}</h3>
