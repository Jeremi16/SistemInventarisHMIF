...
<div class="mx-auto max-w-3xl space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Catat Barang Masuk</h2>
            <p class="mt-1 text-sm text-gray-500">Stok barang akan bertambah setelah transaksi disimpan.</p>
        </div>
        <a href="{{ route('incoming.index') }}" class="inline-flex items-center justify-center rounded-lg border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50">Kembali</a>
    </div>
    ...
    <form method="POST" action="{{ route('incoming.store') }}" enctype="multipart/form-data" class="rounded-xl border border-gray-100 bg-white p-6 shadow-sm">
        @csrf
